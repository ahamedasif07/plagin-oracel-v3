...
?>
        ...
        <style id="oracle-booking-inline-css">
            <?php echo file_exists($css_path) ? file_get_contents($css_path) : '';
            ?>
        </style>
        <script>
            var oracleBooking = {
        ...
        <script id="oracle-booking-inline-js">
            <?php echo file_exists($js_path) ? file_get_contents($js_path) : ''; ?>
        </script>
    <?php
    }
    ?>
    <div class="oracle-booking-wrap w-full lg:max-w-7xl xl:max-w-[1100px] mx-auto">
            ...
            <?php foreach ($stepNames as $i => $s): ?>
                <div class="flex items-center gap-2 sm:gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <span
                            class="step-label hidden text-xs uppercase tracking-widest sm:inline <?php echo $i === 0 ? 'text-gold' : 'text-muted-foreground'; ?>"><?php echo esc_html($s); ?></span>
                    </div>
        ...
        <div id="ob-success-overlay" class="ob-overlay hidden">
            <div class="ob-success-card">
                ...
                <p class="ob-success-text">Thank you. Our team will review your request and, once accepted, you'll receive a
                    confirmation email with your booking invoice.</p>
            </div>
        </div>
        ...
        <form id="booking-form" class="glass-card space-y-6 rounded-3xl p-8 md:p-12" method="post" action="" novalidate>
            ...
            <div class="booking-step active grid gap-4 sm:grid-cols-3" data-step="0">
                <?php foreach ($vehicleOptions as $i => $v): ?>
                    <label
                        ...
                        <input type="radio" name="vehicle" value="<?php echo esc_attr($v['name']); ?>"
                            data-price="<?php echo esc_attr($v['price']); ?>" <?php echo $i === 0 ? 'checked' : ''; ?>
                ...
                <div class="sm:col-span-3">
                    <label class="mb-2 block text-xs uppercase text-muted-foreground">Special
                        Requests</label>
                    <textarea name="special_requests" rows="3" placeholder="Child seat, additional stop, etc."
                        class="<?php echo esc_attr($inputCls); ?>"></textarea>
                </div>
            </div>
            ...
            <div class="booking-step grid gap-6 lg:grid-cols-2" data-step="1">
                ...
                <div class="grid content-start gap-5 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2 ob-address-field">
                        <label class="mb-2 block text-xs uppercase text-muted-foreground">Pickup
                            Address</label>
                        <input type="text" id="ob-pickup-address" name="pickup_address"
                            placeholder="e.g. 10 Downing St, London" autocomplete="off"
                            class="<?php echo esc_attr($inputCls); ?>" required>
                    ...
                    <div class="sm:col-span-2 ob-address-field">
                        <label class="mb-2 block text-xs uppercase text-muted-foreground">Destination</label>
                        <input type="text" id="ob-destination-address" name="destination"
                            placeholder="e.g. Heathrow Terminal 5" autocomplete="off"
                            class="<?php echo esc_attr($inputCls); ?>" required>
                    ...
                    <!-- Other info last -->
                    <div>
                        <label class="mb-2 block text-xs uppercase text-muted-foreground">Journey
                            Type</label>
                    ...
                    <div>
                        <label class="mb-2 block text-xs uppercase text-muted-foreground">Number of
                            Passengers</label>
                        <input type="number" name="passengers" min="1" max="8" value="1"
                            class="<?php echo esc_attr($inputCls); ?>" required>
                    </div>
                    <div>
                        <label class="mb-2 block text-xs uppercase text-muted-foreground">Suitcases</label>
                        <input type="number" name="suitcases" min="0" max="10" value="1"
                            class="<?php echo esc_attr($inputCls); ?>">
                    </div>
                    <div>
                        <label class="mb-2 block text-xs uppercase text-muted-foreground">Flight Number
                            (optional)</label>
                        <input type="text" name="flight_number" placeholder="e.g. BA286"
                            class="<?php echo esc_attr($inputCls); ?>">
                    </div>
                </div>
            ...
            <div class="booking-step grid gap-5 md:grid-cols-2" data-step="2">
                <div>
                    <label class="mb-2 block text-xs uppercase text-muted-foreground">Full Name</label>
                    <input type="text" name="full_name" placeholder="Enter your name"
                        class="<?php echo esc_attr($inputCls); ?> !text-black !bg-white placeholder:!text-muted-foreground"
                        required>
                </div>
                <div>
                    <label class="mb-2 block text-xs uppercase text-muted-foreground">Phone</label>
                    <input type="tel" name="phone" placeholder="Enter your phone"
                        class="<?php echo esc_attr($inputCls); ?> !text-black !bg-white placeholder:!text-muted-foreground"
                        required>
                </div>
                <div class="md:col-span-2">
                    <label class="mb-2 block text-xs uppercase text-muted-foreground">Email</label>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 pt-4">
                ...
                <button type="submit" id="booking-next"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ob-btn-arrow" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M5 12h14" />
                        <path d="m12 5 7 7-7 7" />
                    </svg>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ob-btn-spinner hidden animate-spin"
